...
class Aplicacion:
    ...
    def mostrar_panel_dueño(self):
        self.ventana_login.withdraw()  # Cierra la ventana de login
    ...
    def mostrar_panel_cajero(self):
        ...
        self.boton_QR_Menu = tk.Button(self.ventana_cajero, text="QR MENU", bg="Red", fg="white", font=("Arial", 12),command=self.qrmenu, width=20, height=2)
    ...
    def qrmenu(self):
        ...
        ventana_imagen = tk.Toplevel(self.ventana_cajero)
        ventana_imagen.title("Imagen QR")

        # Cargar la imagen con Pillow (asegúrate de que la ruta sea correcta)
        imagen = Image.open("menu.jpg")
        ...
        # Crear un Label para mostrar la imagen en la nueva ventana
    ...
    def volver_info_pedidos(self):
        ...
        self.pedido_especifico.withdraw()
    ...
    def mostrar_panel_stock(self):
        ...
        # Aplica fondo a la ventana
        aplicar_fondo(self.ventana_stock)

        # Botón para agregar un producto
        boton_agregar_producto = tk.Buttonboton_agregar_producto = tk.Button(self.ventana_stock, text="Agregar Producto", command=self.agregar_producto, width=20, height=2, bg="#FF4C4C", fg="white", font=("Arial", 14, "bold"), relief="raised", bd=3)
        boton_agregar_producto.pack(pady=20)  # Un poco más de espacio entre botones
        # Botón para ver los pedidos
        boton_pedidos = tk.Button(self.ventana_stock, text="Ver Pedidos", command=self.pedidos_stock, width=20, height=2, bg="#FF4C4C", fg="white", font=("Arial", 14, "bold"), relief="raised", bd=3)
        boton_pedidos.pack(pady=20)
        
        # Botón "Volver Atrás"
        boton_volver_atras = tk.Button(self.ventana_stock, text="Volver Atrás", command=self.volver_login_stock, width=10, height=2, bg="#FF4C4C", fg="white", font=("Arial", 14, "bold"), relief="raised", bd=3)
        boton_volver_atras.pack(pady=20)
    ...
    def agregar_producto(self):
        ...
        Enviar = tk.Button(self.ventana_agregar_producto, text="Enviar", command=self.guardar_producto_nuevo, bg="#4CAF50", fg="white", font=("Arial", 14, "bold"), width=10, height=1, relief="raised", bd=3)
        Enviar.pack(pady=10)

        Volver = tk.Button(self.ventana_agregar_producto, text="Volver atrás", command=self.volver_panel_stock_productos, bg="#F44336", fg="white", font=("Arial", 14, "bold"), width=10, height=1, relief="raised", bd=3)
        Volver.pack(pady=10)
    ...
    def pedidos_stock(self):
        ...
        realizar_pedido = tk.Button(
            ...
            activeforeground="white",  # Color de texto cuando el botón es presionado
            padx=15,  # Relleno en los lados
            pady=8   # Relleno arriba y abajo
        )
        realizar_pedido.grid(row=0, columnspan=2, pady=20)

        # Leer el archivo pedidos.json para obtener los datos
        ...
        if os.path.exists(pedidos_file):
            ...
            pedidos = {}
            cantidad_de_pedidos = 0  # Si no existe, asumimos que no hay pedidos

        # Crear botones para cada pedido con sus datos
        for i, (pedido_id, pedido_data) in enumerate(pedidos.items(), start=1):
            # Crear el texto del botón con los productos y el total
            productos = ", ".join([f"{producto} (x{cantidad})" for producto, cantidad in pedido_data["productos"].items()])
            total = pedido_data["total"]
            texto_boton = f"{pedido_id.capitalize()}:\nProductos: {productos}\nTotal: ${total:.2f}"

            pedido_button = tk.Button(
                self.pedidos,
                text=f"Pedido {i}",
                command=lambda info=texto_boton: self.pedido_stock(info),
                bg="#4CAF50",  # Verde
                fg="white",  # Texto blanco
                font=("Arial", 14, "bold"),  # Fuente más grande y en negrita
                width=20,  # Ancho ajustado
                height=2,  # Altura ajustada
                relief="raised",  # Borde elevado
                bd=4,  # Grosor del borde
            ...
            pedido_button.grid(row=(i-1)//2+1, column=(i-1)%2, padx=10, pady=10)

        volver = tk.Button(
            self.pedidos,
            text="Volver atrás",
            command=self.volver_panel_stock,
            bg="#F44336",  # Rojo
            fg="white",  # Texto blanco
            font=("Arial", 14, "bold"),  # Fuente más grande y en negrita
            width=20,  # Ancho ajustado
        ...
        volver.grid(row=(cantidad_de_pedidos//2)+2, columnspan=2, pady=20)
    ...
    def Realizar_pedido(self):
        ...
        boton_ver_productos = tk.Button(
            ...
            font=("Arial", 12, "bold"),  # Fuente en negrita
        ...
        boton_volver = tk.Button(
            self.ventana_pedido_nuevo,
            text="Volver atrás",
            command=self.de_pedido_especifico_a_pedidos,
            width=20,
            height=2,
    ...
    def finalizar_pedido(self):
        ...
        self.ventana_pedido_nuevo.withdraw()  # Cierra la ventana del pedido
    ...
    def pedido_stock(self, info_especifica_stock):
        ...
        self.pedido_especifico_stock.geometry("300x300")
        ...
        volver = tk.Button(
            self.pedido_especifico_stock, 
            text="Volver atrás", 
            command=self.volver_pedidos_stock,
            bg="#F44336",  # Rojo
            fg="white",  # Texto blanco
            font=("Arial", 12, "bold"),  # Fuente en negrita para mayor visibilidad
            width=20,  # Ancho moderado
            height=2,  # Altura ajustada
            relief="raised",  # Borde elevado
            bd=4,  # Grosor del borde
            activebackground="#D32F2F",  # Rojo más oscuro al presionar
            activeforeground="white",  # Texto blanco cuando es presionado
            padx=15,  # Relleno lateral
            pady=8   # Relleno arriba y abajo
        )
        volver.pack(pady=10)
    ...
    def crear_cuenta_empleado(self):
            ...
            self.ventana_crear.title("Crear Cuenta Empleado")
            self.ventana_crear.geometry("450x300")
            
            aplicar_fondo(self.ventana_crear)

            tk.Label(self.ventana_crear, text="Nombre de usuario", font=("Arial", 12)).pack(pady=5)
            entrada_usuario = tk.Entry(self.ventana_crear, font=("Arial", 12))
            entrada_usuario.pack()

            tk.Label(self.ventana_crear, text="Contraseña", font=("Arial", 12)).pack(pady=5)
            entrada_contraseña = tk.Entry(self.ventana_crear, font=("Arial", 12))
            entrada_contraseña.pack()

            tk.Label(self.ventana_crear, text="Rol", font=("Arial", 12)).pack(pady=5)
            entrada_rol = tk.Entry(self.ventana_crear, font=("Arial", 12))
            entrada_rol.pack()
            ...
            def guardar():
                ...
                empleados = cargar_empleados()
                empleados[usuario] = {"contraseña": contraseña, "rol": rol, "sueldo": 200000}
                guardar_empleados(empleados)

                messagebox.showinfo("Éxito", f"Empleado {usuario} creado con éxito.")
            

            boton_guardar = tk.Button(self.ventana_crear, text="Guardar", command=guardar, bg="#4CAF50", fg="white", font=("Arial", 12), width=10)
            boton_guardar.pack(pady=10)

            # Botón de volver atrás
            boton_volver_atras = tk.Button(self.ventana_crear, text="Volver Atrás", command=self.volver_ventana_dueño, bg="#FF4C4C", fg="white", font=("Arial", 12, "bold"), width=20, height=2, relief="raised", bd=3)
            boton_volver_atras.pack(pady=20)

    def volver_ventana_dueño(self):
    ...
    def mostrar_empleados_para_modificar_sueldo(self):
        ...
        self.ventana_dueño.withdraw()  # Destruye la ventana principal.
    ...
    def modificar_sueldo(self, empleado, ventana_anterior):
        ventana_anterior.withdraw()  # Destruir la ventana anterior.
        ...
        def guardar_sueldo():
            ...
            ventana_sueldo.withdraw()  # Cerrar la ventana de modificación.
    ...
    def mostrar_empleados_para_despedir(self):
        ...
        self.ventana_dueño.withdraw()  # Cierra la ventana del dueño
    ...
    def despedir_empleado(self, empleado):
        ...
        if empleado in empleados:
            ...
            self.ventana_despedir.withdraw()
